...
from bitmovin_api_sdk import CmafMuxing
...
from bitmovin_api_sdk import DashCmafRepresentation, DashRepresentationType, DashRepresentationTypeMode
...
TEST_ITEM = "live-srt-ingest-h264-aac-cmaf-ll-hls-dash"
...
def main():
    ...
    for video_profile in video_encoding_profiles:
        ...
        # 低遅延ライブ用に小さい keyframe interval を設定し、LIVE_LOW_LATENCY プリセットを使用します。
        # Use a small keyframe interval and the LIVE_LOW_LATENCY preset for low-latency live.
        h264_codec = bitmovin_api.encoding.configurations.video.h264.create(
            h264_video_configuration=H264VideoConfiguration(
                ...
                preset_configuration=PresetConfiguration.LIVE_LOW_LATENCY
            )
        )
        ...
        # Create a chunked CMAF Muxing for this particular resolution.
        # frames_per_cmaf_chunk は CMAF チャンク(LL の "part")の粒度を制御します。
        # frames_per_cmaf_chunk controls the CMAF chunk (LL "part") granularity —
        # smaller chunks = lower latency; choose relative to the source frame rate (e.g. ~0.5 s of frames).
        bitmovin_api.encoding.encodings.muxings.cmaf.create(
            encoding_id=encoding.id,
            cmaf_muxing=CmafMuxing(
                segment_length=4,
                segment_naming='segment_%number%.m4s',
                init_segment_name='init.mp4',
                frames_per_cmaf_chunk=15,
                streams=[MuxingStream(stream_id=h264_stream.id)],
                outputs=[video_muxing_output],
                name=f"Video CMAF Muxing {video_profile.get('height')}p"
            )
        )
    ...
    for audio_profile in audio_encoding_profiles:
        ...
        Finally, create a chunked CMAF muxing for each variant.
        """
        ...
        # Create a chunked CMAF muxing for the audio variant.
        # frames_per_cmaf_chunk は CMAF チャンク(LL の "part")の粒度を制御します。
        # frames_per_cmaf_chunk controls the CMAF chunk (LL "part") granularity —
        # smaller chunks = lower latency; choose relative to the source frame rate (e.g. ~0.5 s of frames).
        bitmovin_api.encoding.encodings.muxings.cmaf.create(
            ...
            cmaf_muxing=CmafMuxing(
                ...
                name=f"Audio CMAF Muxing {audio_profile.get('bitrate') / 1000:.0f}kbps"
            )
        )
    ...
    # 低遅延再生では小さい live_edge_offset を指定し、ライブエッジに近い位置から再生を開始します。
    # For low latency, use a small live_edge_offset so playback starts close to the live edge.
    # NOTE: True LL playback requires an LL-HLS/LL-DASH capable player and a CDN that supports
...
def _create_hls_manifest(encoding_id, output, output_path):
    """
    Create an HLS manifest using the generated CMAF muxings.
    Loop through all CMAF muxings and add audio or video entries to the HLS manifest.
    ...
    # Retrieve all CMAF muxings from the encoding,
    # then match them to their streams, and add them to the HLS manifest.
    cmaf_muxings = bitmovin_api.encoding.encodings.muxings.cmaf.list(encoding_id=encoding_id)
    for muxing in cmaf_muxings.items:
...
def _create_dash_manifest(encoding_id, output, output_path):
    ...
    and appending CMAF representations for each muxing/stream combination.
...
def _remove_output_base_path(text):
    ...
    :param text: The full path (e.g., 'output/h264-aac-cmaf-ll-hls-dash/video/720p')

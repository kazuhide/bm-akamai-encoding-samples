...
from bitmovin_api_sdk import S3AccessStyle, S3SignatureVersion, GenericS3Output
...
from bitmovin_api_sdk import RedundantRtmpInput, RtmpIngestPoint
from bitmovin_api_sdk import Status

TEST_ITEM = "live-rtmp-redundant-ingest-h264-vbr-aac-fmp4-hls-dash"
...
def main():
    ...
    # Create a Redundant RTMP input that exposes two ingest points (main + backup).
    # 冗長化された RTMP 入力を作成し、メイン/バックアップの 2 つの取り込みポイントを定義する。
    # If the main ingest stalls beyond delay_threshold seconds, the encoder fails over to the backup.
    redundant_rtmp_input = bitmovin_api.encoding.inputs.redundant_rtmp.create(
        redundant_rtmp_input=RedundantRtmpInput(
            name='Test Redundant RTMP Input',
            delay_threshold=10,
            ingest_points=[
                RtmpIngestPoint(application_name='live', stream_key='primary'),
                RtmpIngestPoint(application_name='live-backup', stream_key='backup'),
            ]
        )
    )
    ...
    for video_profile in video_encoding_profiles:
        ...
        h264_stream = bitmovin_api.encoding.encodings.streams.create(
            ...
            stream=Stream(
                ...
                input_streams=[StreamInput(
                    input_id=redundant_rtmp_input.id,
                    input_path="live",
                    position=0)],
        ...
        # Define the output path for the final video segments
    ...
    # stream_key is "notused" here: stream keys are defined per ingest point on the redundant input,
    # so the start request's stream_key is ignored. ストリームキーは取り込みポイント側で定義済みのため未使用。
    ...
    # Print both ingest endpoints (main + backup) for the redundant RTMP input.
    # 冗長化された RTMP 入力のメイン/バックアップ両方の取り込み先を表示する。
    print("Live encoding is up and ready for ingest. RTMP ingest endpoints (main + backup):")
    for ingest_point in redundant_rtmp_input.ingest_points:
        print(f"  rtmp://{live_encoding.encoder_ip}/{ingest_point.application_name}/{ingest_point.stream_key}")
...
def _create_hls_manifest(encoding_id, output, output_path):
    ...
    :param output: An output object to specify the target output location.
...
def _create_dash_manifest(encoding_id, output, output_path):
    ...
    :param output: An output for the manifest's final location.
